let () =
  let file =
    ...
    | Some `Stdin ->
        ...
        let contents = Rethfl.Util.In_channel.(input_all stdin) in
        Rethfl.Util.Out_channel.with_file tmp_file ~f:begin fun ch ->
          Rethfl.Util.Out_channel.output_string ch contents
        end;
        tmp_file
    | None -> exit 1
  in
